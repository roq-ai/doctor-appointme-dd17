...
import { DoctorListPage as ShowCaseList } from 'pages/doctors';
...
function DashboardPage() {
  ...
  const doctorStat = useSWR<PaginatedInterface<DoctorInterface>>(
    () => `/doctors`,
    () => getDoctors({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Doctors',
      result: doctorStat?.data?.totalCount,
      isLoading: doctorStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Services',
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`doctors`} passHref legacyBehavior>
                  <Button
